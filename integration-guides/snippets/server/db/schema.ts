...
export const referralRedemption = sqliteTable(
  ...
  {
    id: text('id').primaryKey(),
    referralId: text('referralId').notNull().references(() => referral.id, { onDelete: 'cascade' }),
    refereeUserId: text('refereeUserId').notNull().references(() => user.id, { onDelete: 'cascade' }),
