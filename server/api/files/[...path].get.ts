...
export default defineEventHandler(async (event) => {
  ...
  const waitUntil = cfCtx ? (p: Promise<unknown>) => cfCtx.waitUntil(p) : (_: Promise<unknown>) => {}
  ...
  const blocks = await Promise.all(
    Array.from({ length: lastBlock - firstBlock + 1 }, (_, i) => fetchBlock(blockCtx, firstBlock + i, total))
  )
