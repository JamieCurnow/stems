...
export async function grantReferrerReward({
  ...
  const referrerStripeCustomerId = (referrer as { stripeCustomerId?: string } | undefined)?.stripeCustomerId
  ...
  const claimed = await db
    ...
    .set({ rewardGrantedAt: Date.now() })
    .where(
      and(eq(schema.referralRedemption.id, redemption.id), isNull(schema.referralRedemption.rewardGrantedAt))
    )
