...
export async function scheduleEmail(
      ...
      if (ns) {
        ...
        await stub.fetch('https://do/schedule', {
          ...
          body: JSON.stringify(buildPayload({
            emailId, userId: userId ?? null, leadId: leadId ?? null,
            props, sendAtMs, dedupeKey, baseUrl
          }))
        })
      }
      return existing.id
  ...
  if (ns && doId) {
    ...
    await stub.fetch('https://do/schedule', {
      ...
      body: JSON.stringify(buildPayload({
        emailId, userId: userId ?? null, leadId: leadId ?? null,
        props, sendAtMs, dedupeKey: dedupeKey ?? null, baseUrl
      }))
    })
  }
...
function buildPayload({
  emailId, userId, leadId, props, sendAtMs, dedupeKey, baseUrl
}: BuildPayloadArgs) {
  return {
    emailId, userId, leadId, props,
    sendAt: sendAtMs, dedupeKey,
...
export async function cancelScheduledEmail(event: H3Event, id: string, reason = 'cancelled') {
  ...
  await db
    ...
    .where(
      and(
        eq(schema.scheduledEmail.id, id),
...
export async function cancelScheduledEmails(event: H3Event, args: CancelByDedupeArgs) {
  ...
  for (const row of toCancel) {
    ...
    await db
      ...
      .where(
        and(
          eq(schema.scheduledEmail.id, row.id),
          eq(schema.scheduledEmail.status, 'scheduled')
        )
      )
  }
...
export async function runScheduledEmailQueue(event: H3Event, { limit = 50 }: RunnerOpts = {}) {
  ...
  const due = await db
    ...
    .where(
      and(
        eq(schema.scheduledEmail.status, 'scheduled'),
        lte(schema.scheduledEmail.sendAt, now)
      )
    )
    .limit(limit)
...
async function resolveRecipientFromRow(
  db: Db,
  row: schema.ScheduledEmailRow
): Promise<RowRecipient | null> {
  if (row.userId) {
